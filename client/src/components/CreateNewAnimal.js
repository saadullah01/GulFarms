...
class CreateNewAnimal extends Component {
  ...
  componentDidMount() {
    if (
      this.props.farms.length <= getId() ||
      !this.props.farm.hasOwnProperty("animalPresets")
    ) {
  ...
  componentDidUpdate = (prevProps, prevState) => {
    if (
      this.props.farm.animalPresets.length !==
        prevProps.farm.animalPresets.length ||
      prevState.modal !== this.state.modal ||
      this.props.allPresets.length !== prevProps.allPresets.length
    ) {
  ...
  validate = (errors) =>{
    if(this.props.errors.message =="Error saving animal preset." ){
      return(
        "Name is required"
      );
    }
        ...
        <Row>
          ...
          <div className="pl-1 pr-1 col-sm-4">
            ...
            >
              ...
              <DropdownMenu>{duration_type}</DropdownMenu>
            </UncontrolledDropdown>
          </div>
        </Row>
      );
...
const getId = () => {
  const id = window.location.href.substring(
    window.location.href.lastIndexOf("farms/") + 6,
...
export default connect(mapStateToProps, { savePreset })(
  withRouter(CreateNewAnimal)
);
